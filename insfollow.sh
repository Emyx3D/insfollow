#!/bin/bash
# insfollow v2.0
# recoded by: Eseigbe Emmanuel
# github.com/emyx3D
# If you use any part from this code, give me the credits, please read the README before use
...
echo -e "\e[1;91m ═╩═\e[1;92m         \e[1;91m╩   by - \e[1;92mBLKGhostMan"
...
printf "\n"
printf "  \e[1;77m[\e[0m\e[1;31m+\e[0m\e[1;77m] This technique consists of following/unfollowing celebgrams\e[0m\n"
...
instagram="25025320"
tundeednut="48215605429"


if [[ ! -e celeb_id ]]; then
printf " %s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n" $dualipa $celebrity $mileycyrus $shawnmendes $tundeednut $katyperry $charlieputh $lelepons $camila_cabello $madonna $leonardodicaprio $ladygaga $taylorswift $instagram $neymar $selena $ariana $beyonce $professor $cristiano $kimkardashian $kendall $therock $kylie $jelopez $messi $blkmazzy > celeb_id
fi
...
hmac=$(echo -n "$data" | openssl dgst -sha256 -hmac "${ig_sig}" | cut -d " " -f2)
printf " \e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;93m Trying to follow celebrity Instagram %s ..." $celeb
...
printf " \e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;77m Sleeping for 2 Minutes...\e[0m\n"
sleep 120
#unfollow
for celeb in $(cat celeb_id); do
data='{"_uuid":"'$guid'", "_uid":"'$username_id'", "user_id":"'$celeb'", "_csrftoken":"'$var2'"}'
hmac=$(echo -n "$data" | openssl dgst -sha256 -hmac "${ig_sig}" | cut -d " " -f2)
printf " \e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;93m Trying to unfollow celebrity Instagram %s ..." $celeb
...
printf "\n \e[1;93m [!] Error, stopping to prevent blocking\n"
...
printf " \e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;77m Sleeping for 70 Seconds for block prevention...\e[0m\n"
sleep 70
